...
def get_system_status():
    ...
    # --- SD Card Status ---
    try:
        # Check mount point carefully - requires correct path in .env
...
def read_log_file(log_path, lines=100):
    ...
    try:
        ...
        if sys.platform.startswith('linux'):
            ...
            # Ensure read happens correctly even on Windows
            log_lines = []
            try:
    ...
    except FileNotFoundError: # Handle tail not found or file gone during execution
        try:
            log_lines = []
            with open(log_path, 'r', encoding='utf-8', errors='ignore') as f:
                log_lines = f.readlines()
            return "".join(log_lines[-lines:])
        except Exception as e_read: return f"Could not read log file ({log_path}): {e_read}"
    except Exception as e_tail: return f"Error reading log file ({log_path}): {e_tail}"
...
def internal_notify():
    ...
    if not allow_access: app.logger.warning(f"Unauthorized notification attempt from {request.remote_addr}"); return jsonify({"status": "error", "message": "Unauthorized"}), 403
    if not request.is_json: app.logger.warning(f"Invalid notification request (not JSON) from {request.remote_addr}"); return jsonify({"status": "error", "message": "Invalid request, JSON required"}), 400
    data = request.get_json(); message = data.get('message'); msg_type = data.get('type', 'info')
    if not message: app.logger.warning(f"Invalid notification request (missing message) from {request.remote_addr}"); return jsonify({"status": "error", "message": "Missing 'message' field"}), 400
    add_notification(message, msg_type); return jsonify({"status": "success", "message": "Notification received"}), 200
...
def diagnostics():
    ...
    if sys.platform.startswith('linux'):
        ...
        except Exception as e: disk_info = f"Error getting disk info for {MONITORED_DISK_PATH}: {e}"; add_notification(f"Diagnostics Error: {e}", "error")
...
def wifi():
    if request.method == 'POST':
        if not sys.platform.startswith('linux'):
            flash("Wi-Fi configuration is only supported on Linux.", "error"); add_notification("Wi-Fi config attempted on non-Linux.", "error")
...
def notifications_route(): # Renamed function
    if request.method == 'POST':
        config = { "smtp_server": request.form.get("smtp_server", ""), "smtp_port": request.form.get("smtp_port", ""), "smtp_username": request.form.get("smtp_username", ""), "smtp_password": request.form.get("smtp_password", ""), "target_email": request.form.get("target_email", "") }
...
def backup_config():
    ...
    except OSError as e: flash(f"Error creating backup dir {CONFIG_BACKUP_PATH}: {e}", "error"); add_notification(f"Backup Error: Cannot create dir {CONFIG_BACKUP_PATH}", "error"); return redirect(url_for('index'))
    ...
    for file_path in files_to_backup:
        if file_path and os.path.exists(file_path):
            base_name = os.path.basename(file_path); dest_path = os.path.join(CONFIG_BACKUP_PATH, f"{base_name}.{timestamp}.bak")
            try: shutil.copy2(file_path, dest_path); backed_up_files.append(dest_path)
            except Exception as e: errors.append(f"Failed to backup {file_path}: {e}"); app.logger.error(f"Backup error for {file_path}: {e}")
        else: errors.append(f"File not found/invalid path, skipped backup: {file_path}")
    if backed_up_files: flash(f"Backed up {len(backed_up_files)} files to {CONFIG_BACKUP_PATH}", "success"); add_notification(f"Config backup successful ({len(backed_up_files)} files).", "success")
    if errors:
...
def update_system():
    ...
    if not sys.platform.startswith('linux'): flash("Update only supported on Linux.", "error"); add_notification("Update attempted on non-Linux.", "error"); return render_template('update.html', output="Update feature only on Linux.")
    try:
        add_notification("Attempting system update via git...", "info"); git_cmd = ['git', 'pull']; result_git = subprocess.run(git_cmd, cwd=PROJECT_DIR, capture_output=True, text=True, check=True, timeout=60)
        ...
        if 'requirements.txt' in result_git.stdout:
            ...
            update_output += f"Pip Install:\n{result_pip.stdout}\n{result_pip.stderr}\n\n"; flash("Pip install OK.", "success"); add_notification("Pip requirements installed.", "success")
        ...
        update_output += f"Service Restart:\n{result_restart.stdout}\n{result_restart.stderr}\n"; flash("Gunicorn restart initiated.", "success"); add_notification("Gunicorn restart initiated.", "success")
...
def run_action(action):
    ...
    scripts = { 'upload': os.path.join(script_dir, 'upload_and_cleanup.sh'), 'offload': os.path.join(script_dir, 'offload.sh'), 'retry': os.path.join(script_dir, 'retry_offload.sh'), 'eject': os.path.join(script_dir, 'safe_eject.sh'), 'reboot': 'sudo /sbin/reboot', 'shutdown': 'sudo /sbin/shutdown now' }
    ...
    if action in scripts:
        ...
        try:
            ...
            elif os.path.exists(cmd) and (cmd.endswith('.sh') and os.access(cmd, os.X_OK)): # Ensure script exists and is executable
                 subprocess.Popen([cmd], env=os.environ.copy(), cwd=script_dir, stdin=subprocess.DEVNULL, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL); flash(f"Action '{action}' started.", "success"); add_notification(f"Action '{action}' started by user.", "info")
            elif not cmd.endswith('.sh'): # Non-script command (should only be reboot/shutdown handled above)
                 errmsg = f"Invalid command action configuration: {action}"; flash(errmsg, "error"); add_notification(errmsg, "error"); app.logger.error(errmsg)
...
def credentials():
    ...
    # Clear session flag on GET request to credentials page
...
if __name__ == '__main__':
    ...
    # Read debug flag from environment, default to False
    use_debug = os.getenv('FLASK_DEBUG', 'false').lower() in ['true', '1']
    # threaded=True is fallback if not using Gunicorn; Gunicorn handles concurrency better
    # Use reloader only if debugging
