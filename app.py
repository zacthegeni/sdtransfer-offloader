...
logging.basicConfig(level=logging.INFO); app.logger.setLevel(logging.INFO)

# --- Helper: Add Notification ---
def add_notification(message, msg_type="info"):
    app.logger.info(f"Queueing notification: [{msg_type}] {message}")
    try: notification_queue.put_nowait({"type": msg_type, "message": message})
    except queue.Full: app.logger.warning("Notification queue is full, dropping message.")

# --- Authentication ---
@auth.verify_password
def verify_password(username, password):
    load_dotenv(dotenv_path, override=True); current_admin_user = os.getenv("ADMIN_USERNAME"); current_admin_pass = os.getenv("ADMIN_PASSWORD")
    if not current_admin_user or not current_admin_pass:
        if request.endpoint in ['credentials', 'static', 'stream', 'internal_notify', 'index']: return "temp_user"
        else: return False
    if username == current_admin_user and password == current_admin_pass: return username
    return False
...
def check_initial_setup():
    ...
    if not current_admin_user or not current_admin_pass:
        ...
        if request.endpoint == 'index':
            if 'creds_warning_shown' not in session: flash("Admin credentials are not set. Please set them via the 'Credentials' page.", "warning"); session['creds_warning_shown'] = True
            return
        elif request.endpoint not in allowed_endpoints: return redirect(url_for('credentials'))
    elif 'creds_warning_shown' in session: session.pop('creds_warning_shown')
...
def load_email_config():
    try:
        with open(EMAIL_CONFIG_PATH, 'r') as f: return json.load(f)
    except FileNotFoundError: return {"smtp_server": "", "smtp_port": "", "smtp_username": "", "smtp_password": "", "target_email": ""}
    except Exception as e: app.logger.error(f"Error loading email config: {e}"); flash(f"Error loading email config: {e}", "error"); add_notification(f"Error loading email config: {e}", "error"); return {"smtp_server": "", "smtp_port": "", "smtp_username": "", "smtp_password": "", "target_email": ""}

def save_email_config(config):
    try:
        with open(EMAIL_CONFIG_PATH, 'w') as f: json.dump(config, f, indent=4)
        flash("Email settings saved.", "success"); add_notification("Email settings saved.", "success")
    except Exception as e: app.logger.error(f"Error saving email config: {e}"); flash(f"Error saving email config: {e}", "error"); add_notification(f"Error saving email config: {e}", "error")

def get_system_status():
    status = {}
    # Disk Usage
    try: usage = shutil.disk_usage(MONITORED_DISK_PATH); status['disk_free'] = usage.free // (1024**2); status['disk_percent'] = int((usage.used / usage.total) * 100) if usage.total > 0 else 0; status['free_space_mb'] = f"{status['disk_free']} MB free"
    except Exception as e: status['free_space_mb'] = f"Disk Error: {e}"; status['disk_percent'] = 'N/A'
    # CPU/Mem
    status['cpu_usage'] = psutil.cpu_percent(interval=0.5); status['mem_usage'] = psutil.virtual_memory().percent
    # SD Card Status - Check path from .env if it was defined
    ...
    if sd_path_to_check: # Only check if the path was actually set in .env
        ...
        except Exception as e: app.logger.error(f"Error checking SD mount status for {sd_path_to_check}: {e}")
    else:
        # If SD_MOUNT_PATH was commented out/empty, status remains Not Detected
        pass # Or maybe try finding *any* /media/pi mount? More complex.

    # Last Run Timestamp
    last_run_file = os.path.join(PROJECT_DIR, "logs/last_run.txt"); status['last_offload_run'] = "Never/Unknown"
    try:
        if os.path.exists(last_run_file):
            with open(last_run_file, 'r') as f: status['last_offload_run'] = f.read().strip()
...
def read_log_file(log_path, lines=100):
    # (Keep the improved read_log_file function from previous versions)
    if not os.path.exists(log_path): return f"Log file not found: {log_path}"
    try:
        if sys.platform.startswith('linux'): process = subprocess.run(['tail', '-n', str(lines), log_path], capture_output=True, text=True, check=False); return process.stdout if process.stdout else "(Log empty or tail error)"
        else: log_lines = []; with open(log_path, 'r', encoding='utf-8', errors='ignore') as f: log_lines = f.readlines(); return "".join(log_lines[-lines:])
    except FileNotFoundError:
        try: log_lines = []; with open(log_path, 'r', encoding='utf-8', errors='ignore') as f: log_lines = f.readlines(); return "".join(log_lines[-lines:])
...
def stream():
    ...
    def event_stream():
        while True:
            try: message_data = notification_queue.get(timeout=60); sse_data = f"data: {json.dumps(message_data)}\n\n"; yield sse_data; notification_queue.task_done()
...
def wifi():
    if request.method == 'POST':
        ...
        except Exception as e: flash(f"Error configuring Wi-Fi: {e}", "error"); add_notification(f"Error configuring Wi-Fi: {e}", "error"); app.logger.error(f"WiFi config failed", exc_info=True)
            ...
            if scan_result.returncode == 0:
                for line in scan_result.stdout.splitlines():
                    ...
                    if "ESSID:" in line_strip:
                        ...
                        except IndexError: app.logger.debug(f"Could not parse SSID from: {line_strip}"); pass
...
def update_system():
    ...
    try:
        ...
        if 'requirements.txt' in result_git.stdout:
            flash("requirements.txt changed, installing...", "info"); add_notification("Installing requirements...", "info"); pip_cmd = [os.path.join(PROJECT_DIR, 'venv/bin/pip'), 'install', '-r', 'requirements.txt']; result_pip = subprocess.run(pip_cmd, cwd=PROJECT_DIR, capture_output=True, text=True, check=True, timeout=180)
        ...
        update_output += f"Service Restart:\n{result_restart.stdout}\n{result_restart.stderr}\n"; flash("Gunicorn restart OK.", "success"); add_notification("Gunicorn restart OK.", "success")
