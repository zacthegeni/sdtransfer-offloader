...
# For debugging, we enable verbose output:
set -ex
...
# Critical paths – adjust these if necessary:
SD_MOUNT="/media/zmakey/251 GB Volume"  # Check that this is the correct mount point for your SD card.
...
if [ ! -d "${LOG_DIR}" ]; then
    echo "[WARN] Log directory ${LOG_DIR} not found. Creating it..."
    mkdir -p "${LOG_DIR}"
    if [ $? -ne 0 ]; then
...
if ! touch "${LOG_DIR}/.writetest" 2>/dev/null; then
    echo "[ERROR] Log directory ${LOG_DIR} is not writable by user $(whoami). Exiting." >&2
    exit 1
else
...
# Logging function (appends to log file and prints to stdout/stderr)
log_msg() {
    local level="$1"
    local message="$2"
    local timestamp
...
# 0. Check for rclone config file
...
for dir in "${LOCAL_VIDEO}" "${LOCAL_PHOTO}"; do
    if ! mkdir -p "${dir}"; then
        log_msg "ERROR" "Failed to create directory ${dir}. Exiting."
        exit 1
    fi
    if ! touch "${dir}/.writetest" 2>/dev/null; then
        log_msg "ERROR" "Directory ${dir} is not writable. Exiting."
        exit 1
    else
        rm "${dir}/.writetest"
    fi
...
# 4. Copy files from SD card to local storage
...
if [ ! -d "${VIDEO_SOURCE}" ]; then
    ...
    if [ ${RSYNC_V_EXIT} -eq 0 ]; then
        ...
        log_msg "ERROR" "Error during video copy (rsync exit code ${RSYNC_V_EXIT})."
        COPY_ERRORS=1
    fi
fi
...
if [ ! -d "${PHOTO_SOURCE}" ]; then
    ...
    if [ ${RSYNC_P_EXIT} -eq 0 ]; then
        ...
        log_msg "ERROR" "Error during photo copy (rsync exit code ${RSYNC_P_EXIT})."
        COPY_ERRORS=1
    fi
fi
...
# 5. Upload local files to Google Drive using rclone
...
# 6. Finalize and exit with appropriate status code
FINAL_EXIT_CODE=0
